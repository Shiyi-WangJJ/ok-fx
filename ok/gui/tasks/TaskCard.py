...
class TaskCard(ConfigCard):
    def __init__(self, task: BaseTask, onetime):
        ...
        if onetime:
            ...
            # 自动执行开关（默认开启，控制是否参与全自动启动）
            self.enable_button = SwitchButton(parent=self)
            self.enable_button.setOnText(self.tr('ON'))
            self.enable_button.setOffText(self.tr('OFF'))
            self.enable_button.checkedChanged.connect(self._on_auto_start_changed)
        else:
        ...
        if task == self.task or self.onetime:
            ...
            if self.onetime:
                ...
                if self.enable_button:
                    auto_start = self.task.config.get('_auto_start', True)
                    self.enable_button.setChecked(auto_start)
            else:
                if self.enable_button:
    ...
    def _on_auto_start_changed(self, checked):
        """自动执行开关切换 — 持久化到 task config"""
        self.task.config['_auto_start'] = checked
